...
cp -n "${MM_MEM_ROOT}/env/env_gitconfig" ~/.gitconfig
cp -n "${MM_MEM_ROOT}/env/env_tmux.conf" ~/.tmux.conf
...
if [[ "$(uname)" == "Darwin" ]]; then
    ...
    sudo dmidecode -t 17 | grep -A3 -e "Size.*GB" | grep -v "Volatile" | grep "GB"
    sudo dmidecode -t 17 | grep "Configured Memory Speed" | head -1 | grep "Memory Speed"
